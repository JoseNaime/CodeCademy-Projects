...
const validateCred = (card) => {
    const cardCopy = [...card];
    let isSecond = false
    for (let j = card.length - 1; j >= 0; j--) {
        if (isSecond) {
            cardCopy[j] = (cardCopy[j] * 2 > 9) ? (cardCopy[j] * 2) - 9 : cardCopy[j] * 2
        }
        isSecond = !isSecond;
    }
    return cardCopy.reduce((a, b) => a + b, 0) % 10 === 0
}

const findInvalidCards = (batch) => {
    return batch.filter(card => !validateCred(card))
}
